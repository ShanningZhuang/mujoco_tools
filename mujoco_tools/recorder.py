...
class VideoRecorder:
    def __init__(
        self,
        camera_name="lateral_camera_angle",
        width=1920,
        height=1080,
        fps=50,
        vision_flags=None,
        ...
        self.camera_names = (
            camera_name.split() if isinstance(camera_name, str) else camera_name
        )
        self.fps = fps
    ...
    def initialize(self, output_path, output_prefix):
        ...
        # Initialize mediapy VideoWriter for streaming
        frame_shape = (self.video_height, self.video_width)  # (height, width)
        self.video_writer = mediapy.VideoWriter(
            path=self.output_path, shape=frame_shape, fps=self.fps
        )
        # Enter the context manager
        self.video_writer = self.video_writer.__enter__()
    ...
    def record_frame(self, model, data):
        ...
        if self.rgb_renderer is None:
            self.rgb_renderer = mujoco.Renderer(
                model, width=self.camera_width, height=self.camera_height
            )

        for camera_name in self.camera_names:
            # Create scene and camera
            self.rgb_renderer.update_scene(
                data, camera=camera_name, scene_option=self.scene_option
            )
        ...
        # Ensure frame is in uint8 format for mediapy
        if combined_frame.dtype != np.uint8:
            combined_frame = (combined_frame * 255).astype(np.uint8)

        # Write frame directly to video (streaming approach)
        self.video_writer.add_image(combined_frame)
    ...
    def _add_activation_map(self, sim_frame, activation):
        ...
        activation_frame = self.visualize_activation(
            activation, shape=self.activation_shape
        )
        ...
        activation_frame_pil = Image.fromarray(activation_frame)
        # Use PIL for high-quality resizing
        activation_frame_resized = np.array(
            activation_frame_pil.resize((act_width, act_height))
        )
    ...
    def save(self, output_path, output_prefix="video"):
        """Finish recording and close video writer"""
    ...
    def visualize_activation(
        ...
        cmap = plt.cm.colors.LinearSegmentedColormap.from_list(
            "custom_cmap", [actuator_color, muscle_color]
        )
        ...
        # Add grid
        plt.grid(
            True, which="both", axis="both", color="black", linestyle="-", linewidth=4
        )
...
class StateRecorder:
    ...
    def save(self, output_path, output_prefix="state"):
        ...
        for datatype in self.datatypes:
            np.save(
                f"{self.output_path}/{self.output_prefix}_{datatype}.npy",
                np.array(self.recorded_data[datatype]),
            )
